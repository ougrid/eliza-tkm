...
import { Connection as Connection5, PublicKey as PublicKey5 } from "@solana/web3.js";
...
import { TrustScoreDatabase as TrustScoreDatabase3 } from "@ai16z/plugin-trustdb";
import {
...
import { settings as settings3 } from "@ai16z/eliza";
...
async function getTokenDecimals(connection3, mintAddress) {
  const mintPublicKey = new PublicKey6(mintAddress);
...
var executeSwap = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      ...
      const walletPublicKey = new PublicKey7(
        runtime.getSetting("SOLANA_PUBLIC_KEY") ?? runtime.getSetting("WALLET_PUBLIC_KEY")
      );
      const provider = new WalletProvider(connection3, walletPublicKey);
      ...
      const expectedPublicKey = runtime.getSetting("SOLANA_PUBLIC_KEY") ?? runtime.getSetting("WALLET_PUBLIC_KEY");
      ...
      if (type === "buy") {
        ...
        const module = await import("./lib-RTEYRPKK.js");
        const Database = module.default;
        const trustScoreDb = new TrustScoreDatabase3(
          new Database(":memory:")
        );
        ...
        const sellTimeStamp = (/* @__PURE__ */ new Date()).getTime().toString();
...
import { Connection as Connection9, Keypair as Keypair4, PublicKey as PublicKey9, Transaction } from "@solana/web3.js";
...
async function promptConfirmation2() {
...
var executeSwapForDAO = {
  ...
  handler: async (runtime, message) => {
    ...
    try {
      ...
      const confirmSwap = await promptConfirmation2();
      if (!confirmSwap) {
...
import { elizaLogger, settings as settings6 } from "@ai16z/eliza";
import {
  Connection as Connection10,
  Keypair as Keypair5,
...
var transfer_default = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      ...
      const senderKeypair = Keypair5.fromSecretKey(secretKey);
      const connection3 = new Connection10(settings6.RPC_URL);
      ...
      const senderATA = getAssociatedTokenAddressSync2(
        mintPubkey,
      ...
      const transaction = new VersionedTransaction3(messageV0);
