...
var TradeSchema = z.object({
  ...
  // Optional leverage for leveraged trades
});
...
// src/utils.ts
import { Coinbase as Coinbase2, Wallet } from "@coinbase/coinbase-sdk";
...
function getCharityAddress(network) {
...
var massPayoutProvider = {
  get: async (runtime, _message) => {
    try {
      ...
      elizaLogger2.log("Reading CSV file from:", csvFilePath);
      if (!fs2.existsSync(csvFilePath)) {
        ...
        elizaLogger2.log("New CSV file created with headers.");
      }
      ...
      elizaLogger2.log("Parsed CSV records:", records);
      elizaLogger2.log("Current Balances:", balances);
      elizaLogger2.log("Last Transactions:", transactions);
      return {
...
async function executeMassPayout(runtime, networkId, receivingAddresses, transferAmount, assetId) {
  ...
  for (const address of receivingAddresses) {
    elizaLogger2.log("Processing payout for address:", address);
    if (address) {
      try {
        const walletBalance = await sendingWallet.getBalance(assetIdLowercase);
        elizaLogger2.log("Wallet balance for asset:", {
          assetId,
          walletBalance
        });
      ...
      elizaLogger2.log("Skipping invalid or empty address.");
      transactions.push({
  ...
  elizaLogger2.log("Finished processing mass payouts.");
  return transactions;
}
var sendMassPayoutAction = {
  ...
  validate: async (runtime, _message) => {
    elizaLogger2.log("Validating runtime and message...");
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger2.log("Starting SEND_MASS_PAYOUT handler...");
    try {
      Coinbase3.configure({
        apiKeyName: runtime.getSetting("COINBASE_API_KEY") ?? process.env.COINBASE_API_KEY,
        privateKey: runtime.getSetting("COINBASE_PRIVATE_KEY") ?? process.env.COINBASE_PRIVATE_KEY
...
- Asset ID is valid.`
      ...
      elizaLogger2.log("\u25CE Starting mass payout...");
...
import {
  ...
  generateObjectV2 as generateObjectV22,
...
var createCoinbaseChargeAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    const chargeDetails = await generateObjectV22({
      runtime,
      context,
    ...
    elizaLogger3.log("Charge details received:", chargeDetails);
    try {
...
var getAllChargesAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    try {
      ...
      elizaLogger3.log("Fetched all charges:", charges);
      callback(
        {
...
var getChargeDetailsAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      ...
      elizaLogger3.log("Fetched charge details:", chargeDetails2);
      callback(
        {
...
var chargeProvider = {
  get: async (runtime, _message) => {
    ...
    if (coinbaseAPIKey && coinbasePrivateKey) {
      ...
      elizaLogger3.log("Current Balances:", balances2);
      elizaLogger3.log("Last Transactions:", transactions2);
    }
    elizaLogger3.log("Charges:", charges);
...
import {
  ...
  generateObjectV2 as generateObjectV23,
...
var tradeProvider = {
  get: async (runtime, _message) => {
    try {
      ...
      elizaLogger4.log("Reading CSV file from:", tradeCsvFilePath2);
      if (!fs3.existsSync(tradeCsvFilePath2)) {
        ...
        await csvWriter.writeRecords([]);
        elizaLogger4.log("New CSV file created with headers.");
      }
      ...
      elizaLogger4.log("Parsed CSV records:", records);
      const { balances, transactions } = await getWalletDetails(runtime);
      elizaLogger4.log("Current Balances:", balances);
      elizaLogger4.log("Last Transactions:", transactions);
      return {
...
var executeTradeAction = {
  ...
  validate: async (runtime, _message) => {
    elizaLogger4.log("Validating runtime for EXECUTE_TRADE...");
    return !!(runtime.character.settings.secrets?.COINBASE_API_KEY || process.env.COINBASE_API_KEY) && !!(runtime.character.settings.secrets?.COINBASE_PRIVATE_KEY || process.env.COINBASE_PRIVATE_KEY);
  },
  handler: async (runtime, _message, state, _options, callback) => {
    elizaLogger4.log("Starting EXECUTE_TRADE handler...");
    try {
      Coinbase5.configure({
        apiKeyName: runtime.getSetting("COINBASE_API_KEY") ?? process.env.COINBASE_API_KEY,
        privateKey: runtime.getSetting("COINBASE_PRIVATE_KEY") ?? process.env.COINBASE_PRIVATE_KEY
      ...
      const tradeDetails = await generateObjectV23({
        runtime,
        context,
...
import { Coinbase as Coinbase6 } from "@coinbase/coinbase-sdk";
import {
  elizaLogger as elizaLogger5,
  composeContext as composeContext4,
  generateObjectV2 as generateObjectV24,
...
var deployTokenContractAction = {
  ...
  validate: async (runtime, _message) => {
    elizaLogger5.log("Validating runtime for DEPLOY_TOKEN_CONTRACT...");
    return !!(runtime.character.settings.secrets?.COINBASE_API_KEY || process.env.COINBASE_API_KEY) && !!(runtime.character.settings.secrets?.COINBASE_PRIVATE_KEY || process.env.COINBASE_PRIVATE_KEY);
  },
  handler: async (runtime, _message, state, _options, callback) => {
    elizaLogger5.log("Starting DEPLOY_TOKEN_CONTRACT handler...");
    try {
      Coinbase6.configure({
        apiKeyName: runtime.getSetting("COINBASE_API_KEY") ?? process.env.COINBASE_API_KEY,
        privateKey: runtime.getSetting("COINBASE_PRIVATE_KEY") ?? process.env.COINBASE_PRIVATE_KEY
      ...
      const contractDetails = await generateObjectV24({
        runtime,
        context,
        modelClass: ModelClass4.SMALL,
        schema: TokenContractSchema
      });
      elizaLogger5.log("Contract details:", contractDetails.object);
      ...
      await contract.wait();
      elizaLogger5.log("Deployment details:", deploymentDetails);
      elizaLogger5.log("Contract deployed successfully:", contract);
...
var invokeContractAction = {
  ...
  validate: async (runtime, _message) => {
    elizaLogger5.log("Validating runtime for INVOKE_CONTRACT...");
    return !!(runtime.character.settings.secrets?.COINBASE_API_KEY || process.env.COINBASE_API_KEY) && !!(runtime.character.settings.secrets?.COINBASE_PRIVATE_KEY || process.env.COINBASE_PRIVATE_KEY);
  },
  handler: async (runtime, _message, state, _options, callback) => {
    elizaLogger5.log("Starting INVOKE_CONTRACT handler...");
    try {
      Coinbase6.configure({
        apiKeyName: runtime.getSetting("COINBASE_API_KEY") ?? process.env.COINBASE_API_KEY,
        privateKey: runtime.getSetting("COINBASE_PRIVATE_KEY") ?? process.env.COINBASE_PRIVATE_KEY
      ...
      const invocationDetails = await generateObjectV24({
        runtime,
        context,
        modelClass: ModelClass4.LARGE,
        schema: ContractInvocationSchema
      });
      elizaLogger5.log("Invocation details:", invocationDetails.object);
      ...
      elizaLogger5.log("Invocation options:", invocationOptions);
...
import {
  ...
  generateObjectV2 as generateObjectV25,
...
var webhookProvider = {
  get: async (runtime, _message) => {
    try {
      ...
      elizaLogger6.log("Listing all webhooks:", resp.data);
      return {
...
var createWebhookAction = {
  ...
  validate: async (runtime, _message) => {
    elizaLogger6.log("Validating runtime for CREATE_WEBHOOK...");
  ...
  handler: async (runtime, _message, state, _options, callback) => {
    elizaLogger6.log("Starting CREATE_WEBHOOK handler...");
    try {
      Coinbase7.configure({
        apiKeyName: runtime.getSetting("COINBASE_API_KEY") ?? process.env.COINBASE_API_KEY,
        privateKey: runtime.getSetting("COINBASE_PRIVATE_KEY") ?? process.env.COINBASE_PRIVATE_KEY
      ...
      const webhookDetails = await generateObjectV25({
        runtime,
        context,
      ...
      elizaLogger6.log("Creating webhook with details:", { networkId, notificationUri, eventType, eventTypeFilter, eventFilters });
      const webhook = await Webhook2.create({ networkId, notificationUri, eventType, eventFilters });
      elizaLogger6.log("Webhook created successfully:", webhook.toString());
      callback(
        {
      ...
      elizaLogger6.log("Webhook appended to CSV successfully");
    } catch (error) {
